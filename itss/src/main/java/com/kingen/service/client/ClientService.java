...
public class ClientService extends CommonService<Client,String>{
	 ...
	 * @throws Exception
	 */
	public void save(String clientId ,ClientContact data) throws Exception{
...
//			List<ClientContact> u = ccdao.find(params);//查询出的用户会回滚？
			List<ClientContact> u = ccdao.find(data,"email");//查询出的用户会回滚？
	 ...
	 * @param c
	 */
	//TODO 可以用EXT标注出来密码是否修改，改了则加密，没改则密码不动它
	public void update(ClientContact c){
		ClientContact t = ccdao.unique(c.getId());
		BeanUtils.copyNotNullProperties(c, t,new String[]{"password"});//密码不更新，密码单独更新
		ccdao.update(t);
	}
	...
	public void delThemCascade(String string, List<String> ids) {
		...
		if("client".equals(string)){
			for(String id :ids ){ //
				dao.delete(id); //删除客户 
				dao.deleteContactByClient(id); //删除联系人
				dao.delRel("client",id);//删除关系
				if(++i%50==0){  //如果是50个客户就直接刷新缓存
					dao.flushAndClear();
				}
			}
			...
			for(String id :ids ){ 
				ccdao.delete(id);//删除客户联系人
				dao.delRel("contact",id);//删除关系
				if(++i%50==0){
					dao.flushAndClear();
				}
			}
