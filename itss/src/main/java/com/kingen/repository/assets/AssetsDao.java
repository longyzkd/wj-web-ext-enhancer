...
@Component
public class AssetsDao extends CommonDao<Assets,String>  {
	...
	public Page<SkillAndScoreVo> findPersonScore(Page<SkillAndScoreVo> page, String userId) {
		...
		StringBuilder sql = new StringBuilder("select s.name as name , score.id as id, score.user_Id as userId,s.id as skillId,score.score as score "
				+ "from t_skill_cat s left join t_person_skill_score score on s.id = score.skill_Id  and score.user_Id = :p1 "); //超级管理员不在页面显示
